...
const shortString = (value, max = 500) => typeof value === 'string' && value.length > 0 && value.length <= max;

export function validateGroundingLiteCall(request) {
  if (!exactKeys(request, ['jsonrpc', 'id', 'method', 'params']) || request.jsonrpc !== '2.0'
    || request.method !== 'tools/call' || !exactKeys(request.params, ['name', 'arguments'])) return false;
  const { name, arguments: args } = request.params;
  if (name === 'search_places') {
    if (!exactKeys(args, ['text_query', 'location_bias', 'language_code', 'region_code'])
      || !shortString(args.text_query) || args.text_query.trim().length < 4) return false;
    ...
    return exactKeys(args.location_bias, ['circle']) && exactKeys(circle, ['center', 'radius_meters'])
      && exactKeys(center, ['latitude', 'longitude'])
      && finite(center.latitude, -90, 90) && finite(center.longitude, -180, 180)
  ...
  if (name === 'compute_routes') {
    ...
    return exactKeys(args, ['origin', 'destination', 'travel_mode'])
      && exactKeys(args.origin, ['lat_lng']) && exactKeys(point, ['latitude', 'longitude'])
      && finite(point.latitude, -90, 90) && finite(point.longitude, -180, 180)
      && exactKeys(args.destination, ['place_id']) && shortString(args.destination.place_id, 256)
      && (args.travel_mode === 'WALK' || args.travel_mode === 'DRIVE');
  }
  if (name === 'lookup_weather') {
    return exactKeys(args, ['location', 'units_system']) && exactKeys(args.location, ['place_id'])
