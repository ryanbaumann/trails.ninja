...
**A2UI v0.9** wire protocol: Gemini emits protocol messages through a
...
- Catalog id: `atlas://maps-agentic-ui-catalog` (`ATLAS_CATALOG_ID`)
...
A surface is built by an **array of messages**, each with `version: "v0.9"` and
exactly one of four operations:

| Message | Shape | Effect |
|---|---|---|
| `createSurface` | `{ surfaceId, catalogId, theme?, sendDataModel? }` | Declares a new surface. |
| `updateComponents` | `{ surfaceId, components: ComponentNode[] }` | Upserts nodes by `id` (last-writer-wins). |
...
| `PlaceCard` | `placeId: Dynamic<string>` — fetches live Places details; shows Google data unaltered |
| `MapPreview` | `lat`, `lng`, `zoom?`, `markers?: [{lat,lng,color?,label?}]`, `label?` — a static-map thumbnail, click flies the camera |
