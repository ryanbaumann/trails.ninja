...
- **Conversational flow and transitions.** Let the next concrete artifact or consequence move the story forward. Use a rhetorical question only when Ryan would actually ask it; canned setup phrases and questions that merely announce the next section read generated.
...
- **Do not turn evidence into an audit memo.** A Field Note can be rigorous without stacking a claim table, glossary, null-filled example, pseudo-rubric, numbered rerun plan, and benchmark survey. Choose the one or two artifacts that changed Ryan's mind, walk the reader through them, and carry the remaining evidence in prose. Tables and lists stay only when the relationship is materially harder to understand without them.
- **Show a trace by following one attempt.** When a post depends on a case, trace, grader, or benchmark, explain those artifacts through one real input and its execution path. A dictionary of definitions is a fallback, not the story.
- **Self-correction stays personal, not prosecutorial.** State what the artifact showed, own the overreach, and move on. Repeated courtroom language, staged verdicts, and polished punchlines make an honest post sound performed.
...
- **Keep generation context out of the canvas.** Prompts, source labels, generation notes, model names, review dates, and process metadata belong in the archived generation record unless they are themselves the evidence shown. The published graphic carries only the labels required to understand its idea.
