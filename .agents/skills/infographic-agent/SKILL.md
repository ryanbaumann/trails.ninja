...
description: >
  ...
  A research agent (gemini-3.8-flash) grounds the topic with Google Search and engineers a precise prompt,
...
1. **Research orchestrator (`gemini-3.8-flash`):** reads the user's topic/content, optionally grounds it with Google Search, and returns the same `PrepareResult` contract as the web app: analysis metadata, exact text strings, and a precise image-generation prompt.
...
1. Ask a research/LLM subagent (e.g. `gemini-3.8-flash`) to produce the web-compatible `PrepareResult` JSON from the user's content.
